    ...
    <div class="page_body">
        ...
        if (gdrcd_filter('get', $_POST['op']) == "send_message")
        {
            if (gdrcd_filter('get', $_POST['multipli']) == 'singolo')
            {
                ...
                {
                    gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . gdrcd_capital_letter(gdrcd_filter('in',
                            $destinat)) . "', NOW(), '" . gdrcd_filter('in', $_POST['testo']) . "')");

                    gdrcd_query("INSERT INTO backmessaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . gdrcd_capital_letter(gdrcd_filter('in',
                            $destinat)) . "', NOW(), '" . gdrcd_filter('in', $_POST['testo']) . "')");
                }//if
            } else
            {
                if ($_POST['multipli'] == 'presenti')
                {
                    ...
                    {
                        gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . $record['nome'] . "', NOW(), '" . gdrcd_filter('in',
                                $_POST['testo']) . "')");
                    }
                } else
                {
                    if ($_POST['multipli'] == 'multiplo')
                    {
                        ...
                        foreach ($check_dest as $destinat)
                        {
                            ...
                            $result = gdrcd_query("SELECT nome FROM personaggio WHERE nome = '" . gdrcd_filter('in',
                                    $destinat) . "'", 'result');
                            if (gdrcd_query($result, 'num_rows') > 0)
                            {
                                $query .= " ('" . $_SESSION['login'] . "', '" . gdrcd_capital_letter(gdrcd_filter('in',
                                        $destinat)) . "', NOW(), '" . gdrcd_filter('in', $_POST['testo']) . "'),";
                            }
                        }
                    ...
                    } else
                    {
                        if ($_POST['multipli'] == "broadcast")
                        {
                            ...
                            {
                                gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . $row['nome'] . "' , NOW(), '" . gdrcd_filter('in',
                                        $_POST['testo']) . "')");
                            }
                        ...
                        {
                            if (is_numeric($_POST['multipli']) === true)
                            {
                                gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . $_POST['multipli'] . "', NOW(), '" . gdrcd_filter('in',
                                        $_POST['testo']) . "')");
                            } else
                            {
                                if (empty($_POST['destinatario']) === false)
                                {
                                    gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . gdrcd_capital_letter(gdrcd_filter('in',
                                            $_POST['destinatario'])) . "', NOW(), '" . gdrcd_filter('in',
                                            $_POST['testo']) . "')");
                            ...
                            <select name="multipli">
                                <option value="singolo" selected>
                                    <?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['messages']['multiple']['single']); ?>
                                </option>
                                <option value="multiplo">
                                    <?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['messages']['multiple']['multiple']); ?>
                                </option>
                                <option value="presenti">
                                    <?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['messages']['multiple']['online']); ?>
                                </option>
                                <?php if ($_SESSION['permessi'] >= MODERATOR)
                                {
                                    ...
                                    <option value="broadcast">
                                        <?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['messages']['multiple']['all']); ?>
                                    </option>
                                <?php } ?>
                            </select>
                        </div>
  ...
  (mittente='" . gdrcd_filter('in', $_SESSION['login']) . "' AND mittente_del=0)) LIMIT 1",
            ...
            } else
            {
                ...
                <div class="read_message_box">
                    <div class="infos">
                        <span class="title"><?php echo gdrcd_filter('out',
                                    $MESSAGE['interface']['messages']['date']) . ": "; ?></span>
 	        <span class="body">
 		    ...
 		    <span class="title"><?php echo gdrcd_filter('out', $MESSAGE['interface']['messages']['sender']) . ": "; ?>
                    ...
                    {
                        gdrcd_query("UPDATE messaggi SET letto = 1 WHERE id = " . gdrcd_filter('num',
                                $_REQUEST['id_messaggio']) . " LIMIT 1");
                    } ?>
                            ...
                            <form action="main.php?page=messages_center"
                                ...
                                <input type="hidden"
                                       ...
                                       value="<?php echo gdrcd_filter('out',
                                           $MESSAGE['interface']['messages']['attachment'] . $record['testo']); ?>"/>
                                <input type="hidden"
                                       name="op"
                                       value="attach"/>
                                <input type="image"
                                       ...
                                       alt="<?php echo gdrcd_filter('out',
                                           $MESSAGE['interface']['messages']['attach']); ?>"
                        ...
                        <div class="read_message_box_form">
                            ...
                            <form action="main.php?page=messages_center" method="post">
                                ...
                                <input type="image"
                                       ...
                                       alt="<?php echo gdrcd_filter('out',
                                           $MESSAGE['interface']['messages']['reply']); ?>"
                                       title="<?php echo gdrcd_filter('out',
                                           $MESSAGE['interface']['messages']['reply']); ?>"/>
                            </form>
                        </div>
...
" . gdrcd_filter('in', $_SESSION['login']) . "' LIMIT 1");
            ...
            if (gdrcd_query("", 'affected') > 0) {
                ...
                <div class="warning">
                    <?php echo gdrcd_filter('out',
                        $PARAMETERS['names']['private_message']['sing'] . $MESSAGE['interface']['messages']['erased']); ?>
                </div>
                <div class="link_back">
                    <a href="main.php?page=messages_center&offset=0"><?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['messages']['go_back']); ?></a>
        ...
        if ($_POST['op'] == 'erase_checked') {
            if (!empty($_POST['ids'])) {
                ...
                if ($aff > 0){
                    ...
                    <div class="warning">
                        <?php echo gdrcd_filter('out',
                            $PARAMETERS['names']['private_message']['plur'] . $MESSAGE['interface']['messages']['all_erased']); ?>
                    </div>
                    <div class="link_back">
                        <a href="main.php?page=messages_center&offset=0"><?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['messages']['go_back']); ?></a>
            ...
            } else
            {
                ...
                <div class="warning">
                    <?php echo gdrcd_filter('out',
                        $PARAMETERS['names']['private_message']['plur'] . $MESSAGE['interface']['messages']['erased']); ?>
                </div>
                <div class="link_back">
                    <a href="main.php?page=messages_center&offset=0"><?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['messages']['go_back']); ?></a>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="elementi_elenco">
                                        ...
                                        echo gdrcd_format_date($quando[0]) . '<br/>' . gdrcd_filter('out',
                                                $MESSAGE['interface']['messages']['time']) . ' ' . gdrcd_format_time($quando[1]);
                                        ?>
                                    </div>
                                </td>
                                <td>
                                    <div class="elementi_elenco">
                                        <a href="main.php?page=messages_center&op=read&id_messaggio=<?php echo $row['id'] ?>"><?php echo gdrcd_filter('out',
                                                substr($row['testo'], 0, 40)); ?>...</a>
                                    </div>
                                </td>
                                <td>
                                    <?php
                                        ...
                                        <div class="controlli_elenco">
                                            <div class="controllo_elenco">
                                                ...
                                                <form action="main.php?page=messages_center" method="post">
                                                    ...
                                                    <input type="hidden" name="reply_dest"
                                                           value="<?php echo $row['mittente']; ?>"/>

                                                    <input type="hidden" name="genitore"
          ...
          <form id="multiple_delete" method="post" action="main.php?page=messages_center" onSubmit="return checked_copy();">
            ...
            <input type="submit" value="Cancella Messaggi Selezionati">
                ...
                } else
                {
                    ...
                    {
                        echo '<div class="warning">' . gdrcd_filter('out',
                                $MESSAGE['interface']['messages']['please_erase']) . '</div>';
                    }
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['messages']['no_message']) . '</div>';
                }
                ?>
                <div class="pager">
